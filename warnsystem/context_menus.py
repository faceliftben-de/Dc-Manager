...
class ReasonEntry(Modal, title="Vergebe eine Verwarnung"):
    reason = TextInput(
        label="Grund",
        style=discord.TextStyle.paragraph,
        max_length=2000,
        required=False,
        placeholder="Optionaler Grund für die Verwarnung",
    )
    duration = TextInput(
        label="Dauer",
        ...
        max_length=100,
        required=False,
        placeholder="Beispiel: 2d12h30m = 2 days, 12 hours, 30 minutes",
    )
    ban_days = TextInput(
        label="Anzahl der Tage mitzunehmender Nachrichten",
        style=discord.TextStyle.short,
        max_length=1,
        required=False,
        placeholder="Maximal 7 Tage",
    )
    ...
    async def on_submit(self, interaction: Interaction["Red"]) -> None:
        ...
        if self.duration.value:
            try:
                ...
                await interaction.response.send_message(_("Ungültige Dauer"), ephemeral=True)
                return
            if not duration:
                await interaction.response.send_message(_("Ungültige Dauer"), ephemeral=True)
                return
        if self.ban_days.value:
            try:
                ...
                await interaction.response.send_message(
                    _("Ungültige Anzahl der Tage"), ephemeral=True
                )
                return
            if ban_days > 7 or ban_days < 1:
                await interaction.response.send_message(
                    _("Die Anzahl der Tage muss zwischen 1 und 7 liegen."), ephemeral=True
                )
                return
        ...
        await interaction.response.edit_message(content=_("Fertig."))
...
async def context_warn(interaction: discord.Interaction["Red"], member: discord.Member):
    ...
    if not cog:
        await interaction.response.send_message(
            _("Das WarnSystem Plugin ist nicht geladen"), ephemeral=True
        )
        return
    if (
        ...
        await interaction.response.send_message(
            _("Du bist nicht berechtigt, dies zu tun."), ephemeral=True
        )
        return
    await interaction.response.send_message(
        _("Bitte wähle eine Aktion aus."),
