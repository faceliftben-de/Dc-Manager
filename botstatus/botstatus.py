...
class Botstatus(commands.Cog):
    ...
    async def botstatus(self, ctx):
        """Setze einen Status, der nicht beim Neustart verschwindet.
        Nutze: [p]botstatus <type> <status> <text>"""
        pass

    @botstatus.group(name="playing", aliases=["game"])
    async def game(self, ctx):
        """Nutze: [p]botstatus playing <status> <text>"""
        pass

    @game.command(name="online")
    async def g_online(self, ctx, *, text: str):
        if len(text) > 128:
            await ctx.send(_("Das Textfeld hat ein Limit von 128 Zeichen."))
        else:
            await self.config.status.set(("playing", "online", text))
            await self.setfunc("playing", "online", text)
            await ctx.send(_("Status gesetzt zu ``Online | Playing {text}``").format(text=text))
    ...
    async def g_idle(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Idle | Playing {text}``").format(text=text))
    ...
    async def g_dnd(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``DND | Playing {text}``").format(text=text))
    ...
    async def g_offline(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Offline | Playing {text}``").format(text=text))

    @botstatus.group()
    async def listening(self, ctx):
        """Nutze: [p]botstatus listening <status> <text>"""
        pass
    ...
    async def l_online(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Online | Listening to {text}``").format(text=text))
    ...
    async def l_idle(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Idle | Listening to {text}``").format(text=text))
    ...
    async def l_dnd(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``DND | Listening to {text}``").format(text=text))
    ...
    async def l_offline(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Offline | Listening to {text}``").format(text=text))

    @botstatus.group()
    async def watching(self, ctx):
        """Nutze: [p]botstatus watching <status> <text>"""
        pass
    ...
    async def w_online(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Online | Watching {text}``").format(text=text))
    ...
    async def w_idle(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Idle | Watching {text}``").format(text=text))
    ...
    async def w_dnd(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``DND | Watching {text}``").format(text=text))
    ...
    async def w_offline(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Offline | Watching {text}``").format(text=text))

    @botstatus.group()
    async def competing(self, ctx):
        """Setze einen Compreting-Status"""
        pass
    ...
    async def c_online(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Online | Competing {text}``").format(text=text))
    ...
    async def c_away(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Away | Competing {text}``").format(text=text))
    ...
    async def c_dnd(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``DND | Competing {text}``").format(text=text))
    ...
    async def c_offline(self, ctx, *, text: str):
        if len(text) > 128:
            ...
            await ctx.send(_("Status gesetzt zu ``Offline | Competing {text}``").format(text=text))

    @botstatus.command()
    async def streaming(self, ctx, streamer: str, text: str):
        """
        Setze einen Streaming-Status. Der Streamer-Parameter muss eine gültige Twitch- oder YouTube-URL sein, damit der Status korrekt angezeigt wird.\n
        Nutze: [p]botstatus streaming <stream url> <text>
        """
        if len(text) > 128:
            await ctx.send(_("Das Textfeld hat ein Limit von 128 Zeichen."))
            return
            ...
            if len(streamer) > 511:
                await ctx.send(_("Das Textfeld hat ein Limit von 511 Zeichen."))
                return
            await self.config.status.set(("streaming", streamer, text))
            await self.setfunc("streaming", streamer, text)
            await ctx.send(
                _("Status gesetzt zu ``streaming {text} with url {url}``").format(
    ...
    async def clear(self, ctx):
        """Lösche den gespeicherten Bot-Status."""
        await self.config.status.set((None, None, None))
        await ctx.send(_("Der gespeicherte Bot-Status wurde gelöscht."))
